...
DWORD __stdcall internal_pthread_thread_start(void *arg) {
  ...
  if /* constexpr */ (sizeof(void *) != sizeof(DWORD)) {
    ...
    if (tmp > std::numeric_limits<DWORD>::max()) {
      throw std::out_of_range("Exit code of the pthread is outside the range "
...
int pthread_create(pthread_t *thread, const pthread_attr_t *attr,
  ...
  startupInfo->thread->handle = CreateThread(
      nullptr, stackSize, internal_pthread_thread_start, startupInfo,
      CREATE_SUSPENDED, &startupInfo->thread->threadID);
...
pthread_t pthread_self() {
  ...
  if (current_thread_self == nullptr) {
    ...
    DuplicateHandle(GetCurrentProcess(), GetCurrentThread(),
                    GetCurrentProcess(), &current_thread_self->handle,
...
DWORD pthread_getw32threadid_np(pthread_t thread) { return thread->threadID; }
...
int pthread_setschedparam(pthread_t thread, int policy,
  ...
  if (GetPriorityClass(GetCurrentProcess()) != REALTIME_PRIORITY_CLASS) {
    ...
    } else if (newPrior < THREAD_PRIORITY_TIME_CRITICAL &&
...
public:
  ...
  bool condition_timed_wait(std::condition_variable_any &cond,
...
timespec_to_time_point(const timespec *t) {
...
int pthread_setspecific(pthread_key_t key, const void *value) {
  try {
    ...
    boost::detail::set_tss_data(realKey,

...
template <typename Value> PtrRange ptr_range_key(Value value) {
  auto const data = value.ptrs;
  return {data, data + value.key_size};
}

template <typename Value> PtrRange ptr_range_mapped(Value value) {
...
template <typename Value> fbstring render_tmpl(Value value) {
  auto const str = demangle(value.tmpl->name());
  auto const off = str.find('<');
  return str.substr(off + 1, str.size() - off - 2);
}

template <typename Value> std::string render(Value value) {

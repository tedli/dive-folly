...
template <typename T> static inline uintptr_t page_for(T *addr) {
...
size_t qfind_first_byte_of_needles16(const StringPieceLite haystack,
  ...
  for (; i < haystack.size(); i += 16) {
    ...
    index = _mm_cmpestri(arr2, int(needles.size()), arr1,
                         int(haystack.size() - i), 0);
    if (index < 16) {
      return i + index;
    }
  }
...
size_t scanHaystackBlock(const StringPieceLite haystack,
  ...
  assert(blockStartIdx + 16 <= haystack.size() ||
  ...
  auto arr2 =
      _mm_loadu_si128(reinterpret_cast<const __m128i *>(needles.data()));
  auto b =
  ...
  for (; j < needles.size(); j += 16) {
    ...
    auto index = _mm_cmpestri(arr2, int(needles.size() - j), arr1,
                              int(haystack.size() - blockStartIdx), 0);
    b = std::min(index, b);
  }

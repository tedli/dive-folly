...
template <> const char *const MaxString<unsigned long>::value = "4294967295";
#else
template <>
...
constexpr const std::array<ErrorString, static_cast<std::size_t>(
...
Expected<Tgt, ConversionCode> str_to_floating(StringPiece *src) noexcept {
  ...
  static StringToDoubleConverter conv(
      ...
      std::numeric_limits<Tgt>::quiet_NaN(), nullptr, nullptr);
  ...
  auto result = std::is_same<Tgt, float>::value
                    ? conv.StringToFloat(src->data(),
                                         static_cast<int>(src->size()), &length)
  ...
  case 'i':
    if (size >= 3 && tolower_ascii(b[1]) == 'n' && tolower_ascii(b[2]) == 'f') {
  ...
  case 'n':
    if (size >= 3 && tolower_ascii(b[1]) == 'a' && tolower_ascii(b[2]) == 'n') {
      b += 3;
 ...
 */
template <typename T, bool IsSigned = is_signed_v<T>> class SignedValueHandler;

template <typename T> class SignedValueHandler<T, true> {
public:
  ...
  template <typename U> Expected<T, ConversionCode> finalize(U value) {
    T rv;
    if (negative_) {
      FOLLY_PUSH_WARNING
...
template <typename T> class SignedValueHandler<T, false> {
public:
 ...
 */
template <class Tgt>
inline Expected<Tgt, ConversionCode> digits_to(const char *b,
...
template Expected<char, ConversionCode> digits_to<char>(const char *,
                                                        const char *) noexcept;
template Expected<signed char, ConversionCode>
digits_to<signed char>(const char *, const char *) noexcept;
template Expected<unsigned char, ConversionCode>
digits_to<unsigned char>(const char *, const char *) noexcept;

template Expected<short, ConversionCode>
...
template Expected<int, ConversionCode> digits_to<int>(const char *,
                                                      const char *) noexcept;
template Expected<unsigned int, ConversionCode>
digits_to<unsigned int>(const char *, const char *) noexcept;

template Expected<long, ConversionCode> digits_to<long>(const char *,
                                                        const char *) noexcept;
template Expected<unsigned long, ConversionCode>
digits_to<unsigned long>(const char *, const char *) noexcept;

template Expected<long long, ConversionCode>
...
#if FOLLY_HAVE_INT128_T
template Expected<__int128, ConversionCode>
...
Expected<Tgt, ConversionCode> str_to_integral(StringPiece *src) noexcept {
  ...
  if (FOLLY_UNLIKELY(!tmp.hasValue())) {
    return makeUnexpected(tmp.error() == ConversionCode::POSITIVE_OVERFLOW
                              ? sgn.overflow()
                              : tmp.error());
  }
...
template Expected<char, ConversionCode>
